...
_PROBE = (
    ...
    "print(config.TRASH_DIR); print(config.project_dir('origenerator'))"
)


def _paths(state_dir: str | None, *, branch_session: bool = False) -> list[Path]:
    env = {k: v for k, v in os.environ.items()
           if k not in ("ORIGENERATOR_STATE_DIR", "ORIGENERATOR_BRANCH_SESSION")}
    if state_dir is not None:
        env["ORIGENERATOR_STATE_DIR"] = state_dir
    if branch_session:
        env["ORIGENERATOR_BRANCH_SESSION"] = "1"
...
def test_the_app_keeps_its_state_in_its_own_checkout():
    state, db, thumbs, ui, project, trash, _primary = _paths(None)
...
def test_a_process_told_where_its_state_is_keeps_every_path_there(tmp_path):
    ...
    state, db, thumbs, ui, _project, trash, _primary = _paths(str(tmp_path / "elsewhere"))
    assert state == tmp_path / "elsewhere"
    assert db.parent == state
    assert thumbs.parent == state
    assert trash.parent == state
    assert ui.parent == state


def test_a_branch_session_shares_the_live_installs_library_and_keeps_its_own_window_state():
    # The library -- its database, thumbnails and trash -- is the live install's
    # wherever the app runs from, so every instance shows every generation, made
    # in a preview or not. What is about this window alone (its saved geometry
    # and tabs, its logs) stays in the checkout that runs it.
    state, db, thumbs, ui, project, trash, primary = _paths(None, branch_session=True)
    library = primary / "state"
    assert state == project / "state"
    assert (db, thumbs, trash) == (library / "origenerator.db", library / "thumbnails",
                                  library / "trash")
    assert ui == state / "ui_state.json"

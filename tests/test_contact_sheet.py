...
from origenerator.gui.contact_sheet import ContactSheet, grid_for, row_counts
...
def test_nothing_to_tile_lays_out_no_grid():
    assert grid_for(0, 400, 300) == (0, 0)
    assert grid_for(4, 0, 300) == (0, 0)
    assert row_counts(4, 0) == []


def test_one_picture_takes_the_whole_pane():
    assert grid_for(1, 400, 300) == (1, 1)
...
def test_the_grid_picks_the_squarest_cells_the_pane_allows():
    ...
    assert grid_for(9, 400, 300) == (3, 3)
    # A wide, short pane wants its rows long and few, not square-ish.
    assert grid_for(9, 1200, 100)[1] == 1


def test_the_grid_the_search_picks_is_the_grid_that_gets_drawn():
    # The column count has to be the one the rows are actually laid at: the
    # remainder is shared out row by row, so a search over columns would score a
    # cell shape nothing is drawn at.
    for count in range(1, 40):
        columns, rows = grid_for(count, 400, 300)
        assert max(row_counts(count, rows)) == columns

...
from origenerator.show_buttons import MODE_BUTTONS
...
def _alls(tree, orientation=LANDSCAPE):
    """What one half's All row holds: its shelves, then its workflow folders."""
    all_row = _top_level(tree, orientation)["All"]
    return {all_row.child(i).text(0): all_row.child(i) for i in range(all_row.childCount())}
...
def _subs(item):
    """A row's own folders, without the shelves a folder of folders leads with."""
    children = (item.child(i) for i in range(item.childCount()))
    return [child for child in children if child.data(0, _GROUP_ROLE) is not None]


def _at(item, *path):
    for index in path:
        item = _subs(item)[index]
    return item


_SHELF_NAMES = ["Latest", "Favorites", "Experiments", "Requests", "Trash"]


def _shelf_names(item):
    """The shelves a folder leads with, in order — none of them a folder."""
    names = []
    for i in range(item.childCount()):
        if item.child(i).data(0, _GROUP_ROLE) is not None:
            break
        names.append(item.child(i).text(0))
    return names


def _workflow_rows(tree, media=None):
    """The workflow rows, which hang under the All row that tops the tree, by
    label — narrowed to one media type ("image"/"video") when ``media`` says so."""
    all_row = _top_level(tree).get("All")
    if all_row is None:
        return {}
    rows = _subs(all_row)
    if media is not None:
        rows = [row for row in rows if _key(row).startswith(f"{media}/")]
    return {row.text(0): row for row in rows}


def _image_workflow(tree):
    """The sole image workflow's row — where a one-recipe gallery starts."""
    (row,) = _workflow_rows(tree, "image").values()
    return row


def _video_workflow(tree):
    """The sole video workflow's row."""
    (row,) = _workflow_rows(tree, "video").values()
    return row


def test_refresh_builds_workflow_model_settings_tree(qtbot):
    rows = [
    ...
    assert set(top) == {"All"}
    assert _shelf_names(top["All"]) == _SHELF_NAMES
    ...
    assert len(_subs(workflow_node)) == 1           # one model
    assert len(_subs(_at(workflow_node, 0))) == 1   # its single "(no add-on)" level
    assert len(_subs(_at(workflow_node, 0, 0))) == 1  # the two seeds collapse


def test_every_folder_of_folders_leads_with_shelves_of_its_own(qtbot):
    view = GalleryView(FakeDB([_i2v_video("v1", "styleA")]))
    qtbot.addWidget(view)
    view.refresh()
    workflow = _video_workflow(view._tree)
    model, lora, source = (_at(workflow, *[0] * depth) for depth in (1, 2, 3))

    for folder in (workflow, model, lora, source):
        assert _shelf_names(folder) == _SHELF_NAMES
    assert _at(source, 0).childCount() == 0
...
def test_tree_rows_carry_a_recipe_level_badge_and_tooltip(qtbot):
    ...
    workflow = _video_workflow(view._tree)
    model = _at(workflow, 0)
    lora = _at(model, 0)
    source = _at(lora, 0)
    settings = _at(source, 0)

    # Each level carries a chip as its row icon — right of the caret, reading as
...
def test_branch_shows_folder_tiles_and_leaf_shows_thumbnails(qtbot):
    ...
    lora = _at(_image_workflow(view._tree), 0, 0)  # "(no add-on)"
...
def _square_names(view):
    return [tile._text for tile in view._scroll.widget().findChildren(FolderTile)]


def test_a_folder_of_folders_shows_its_shelves_as_squares_before_its_folders(qtbot):
    view = GalleryView(FakeDB([_image("i1", "a cat", 50, 1), _image("i2", "a dog", 50, 1)]))
    qtbot.addWidget(view)
    view.refresh()
    ...
    assert _square_names(view) == [*_SHELF_NAMES, *(row.text(0) for row in _subs(lora))]


def test_clicking_a_shelf_square_opens_that_folders_shelf(qtbot):
    view = GalleryView(FakeDB([_image("i1", "a cat", 50, 1)]))
    qtbot.addWidget(view)
    view.refresh()
    workflow = _image_workflow(view._tree)
    view._tree.setCurrentItem(workflow)
    (trash,) = [tile for tile in view._scroll.widget().findChildren(FolderTile)
                if tile._text == "Trash"]

    trash.clicked.emit(trash.key)

    assert view._tree.currentItem() is _shelf_row(workflow, "Trash")
...
def test_clicking_a_folder_tile_drills_into_it(qtbot):
    ...
    view._browser._drill_into(a_tile_key)  # same path the tile's clicked signal triggers
...
def test_search_leaves_the_tree_exactly_as_it_was(qtbot):
    ...
    assert not workflow.isExpanded()
...
def test_the_tree_selection_is_what_the_search_covers(qtbot):
    ...
    view._tree.setCurrentItem(_video_workflow(view._tree))
    _search_for(view, "cat")
...
def test_the_all_row_searches_the_whole_library(qtbot):
    ...
    # does, so without a row above the workflow folders there is nowhere to
...
def test_the_all_row_holds_the_workflow_folders_and_is_where_the_gallery_lands(qtbot):
    rows = [_image("i1", "a cat", 50, 1), _i2v_video("v1", "styleA")]
    view = GalleryView(FakeDB(rows))
    qtbot.addWidget(view)
    view.refresh()
    ...
    assert [row.text(0) for row in _subs(all_row)] == [
...
def test_a_folders_shelf_scopes_the_search_and_names_its_path(qtbot):
    rows = [_image("i1", "a cat", 50, 1),
            _row("v1", "wan22_i2v", {"positive_prompt": "a cat", "seed": 5},
                 "wan22_i2v_00001_.mp4")]
    view = GalleryView(FakeDB(rows))
    qtbot.addWidget(view)
    view._search.field.setFixedWidth(2000)
    view.refresh()
    workflow = _image_workflow(view._tree)
    ...
    assert view.visible_prompt_ids() == ["i1"]
    assert view._search.field.placeholderText() == (
        f"Search {ORIENTATION_LABELS[LANDSCAPE]}  ›  All  ›  {workflow.text(0)}  ›  Latest…")


def _shelf_row(folder, label):
    (row,) = [folder.child(i) for i in range(folder.childCount())
              if folder.child(i).data(0, _GROUP_ROLE) is None and folder.child(i).text(0) == label]
    return row


def test_a_folders_shelf_row_counts_what_that_folder_holds(qtbot):
    view = GalleryView(_bin_db(rows=[_image("i1", "a cat", 50, 1), _i2v_video("v1", "styleA")],
                               held=[("d1", _image("d1", "a cat", 50, 2))]))
    qtbot.addWidget(view)
    view.refresh()

    assert _shown(_shelf_row(_image_workflow(view._tree), "Trash")) == "(1) Trash"
    assert _shown(_shelf_row(_video_workflow(view._tree), "Trash")) == "Trash"


def test_starring_a_picture_counts_it_on_the_favorites_of_every_folder_above_it(qtbot):
    view = GalleryView(FakeDB([_image("i1", "a cat", 50, 1), _i2v_video("v1", "styleA")]))
    qtbot.addWidget(view)
    view.refresh()

    view.set_items_favorite(["i1"], True)

    assert _shown(_shelf_row(_at(_image_workflow(view._tree), 0), "Favorites")) == "(1) Favorites"
    assert _shown(_shelf_row(_video_workflow(view._tree), "Favorites")) == "Favorites"
...
def test_the_field_says_what_it_would_search(qtbot):
    ...
    assert view._search.field.placeholderText() == "Search Landscape  ›  All  ›  Latest…"
...
def test_an_empty_search_names_the_folder_it_looked_in(qtbot):
    ...
    assert videos.text(0) in view._scroll.widget().text()
...
def test_search_matches_a_prompt_word_no_folder_name_carries(qtbot):
    ...
    view.refresh()
    lora = _at(_image_workflow(view._tree), 0, 0)
    names = [row.text(0) for row in _subs(lora)]
...
def test_opening_a_result_folder_puts_the_search_away(qtbot):
    ...
    view.refresh()
    _search_for(view, "cat")
    (key,) = view._browser._visible_keys
...
def test_favoriting_a_folder_persists_without_reordering(qtbot):
    ...
    lora = _at(_image_workflow(view._tree), 0, 0)  # "(no add-on)"
    cat_key = _key(_at(lora, 0))
    dog_key = _key(_at(lora, 1))  # cat is first, dog second
    ...
    assert [_key(row) for row in _subs(lora)] == [cat_key, dog_key]
    ...
    assert _at(lora, 1).data(0, _GROUP_ROLE).favorite is True
    assert _at(lora, 0).data(0, _GROUP_ROLE).favorite is False
    assert not _at(lora, 1).text(0).startswith("★")
...
def test_favorite_shelf_is_pinned_first_and_collects_favorite_folders(qtbot):
    ...
    lora = _at(_image_workflow(view._tree), 0, 0)  # "(no add-on)"
    dog_key = _key(_at(lora, 1))
    view._toggle_favorite(dog_key)

    # The Favorites shelf sits just below Recents, above the media folders.
    assert list(_alls(view._tree))[:2] == ["Latest", "Favorites"]
...
def test_favorites_leads_with_its_count_and_latest_leaves_its_count_to_all(qtbot):
    ...
    alls = _alls(view._tree)
    assert _shown(alls["Favorites"]) == "(2) Favorites"
    assert _shown(_top_level(view._tree)["All"]) == "(3) All"
    assert _shown(alls["Latest"]) == "Latest"
...
def test_experiments_shelf_label_counts_the_unreviewed(qtbot):
    ...
    assert _shown(_alls(view._tree)["Experiments"]) == "(2) Experiments"
...
def test_experiments_shelf_offers_keep_and_reject_on_each_tile(qtbot):
    ...
    tile = view._browser._thumb_widgets["e1"]
...
def test_double_clicking_an_experiment_opens_it_selected_in_its_own_folder(qtbot):
    ...
    view._tree.setCurrentItem(_shelf(view, EXPERIMENTS_KEY))
    view._browser._thumb_widgets["e1"].double_clicked.emit("e1")
    ...
    assert view._browser._thumb_widgets["e1"].is_selected()
...
def test_keeping_an_experiment_clears_it_from_the_review_queue(qtbot):
    ...
    assert "SDXL Text-to-Image" in _workflow_rows(view._tree)
    ...
    assert "SDXL Text-to-Image" in _workflow_rows(view._tree)       # ...and still in its folder
...
def test_the_trash_shelf_label_counts_what_it_holds(qtbot):
    ...
    assert _shown(_alls(view._tree)["Trash"]) == "(2) Trash"
...
def test_each_trash_tile_offers_restore_and_permanent_delete(qtbot):
    ...
    view._tree.setCurrentItem(_shelf(view, TRASH_KEY))
    tooltips = [b.toolTip() for b in view._browser._thumb_widgets["d1"]._corner_buttons]
...
def test_restoring_from_a_tile_hands_the_id_to_the_action(qtbot):
    ...
    view._browser._thumb_widgets["d1"].corner_action_triggered.emit("d1", "restore")
...
def test_a_restore_puts_the_item_back_in_its_own_folder(qtbot, tmp_path):
    ...
    assert "SDXL Text-to-Image" in _workflow_rows(view._tree)      # back in the tree...
...
def test_purging_asks_before_it_ends_anything(qtbot, monkeypatch):
    ...
    view._browser._thumb_widgets["d1"].corner_action_triggered.emit("d1", "purge")
...
def _trash_shelf_view(qtbot, actions, held=(("d1", "a cat"),)):
    ...
    view.refresh()
    view._tree.setCurrentItem(_shelf(view, TRASH_KEY))
    view._browser.apply_selection(held[0][0], _NO_MOD)
    return view
...
def test_double_clicking_a_deleted_tile_keeps_its_tab(qtbot):
    ...
    view._browser._thumbnail_double_clicked("d1")
...
def test_favorite_shelf_shows_empty_state_when_nothing_is_favorite(qtbot):
    ...
    shelf = _alls(view._tree)["Favorites"]
    view._tree.setCurrentItem(shelf)
    assert view._browser._visible_keys == []   # no tiles, just the hint
    assert view.visible_prompt_ids() == []
...
def test_favorite_shelf_stays_selected_across_a_refresh(qtbot):
    ...
    qtbot.addWidget(view)
    view.refresh()
    view._tree.setCurrentItem(_alls(view._tree)["Favorites"])
...
def test_favorite_shelf_is_absent_until_a_folder_exists(qtbot):
    ...
    view.refresh()
    alls = _alls(view._tree)
    assert "Latest" not in alls
    assert "Favorites" not in alls


def test_a_folders_latest_lists_what_that_folder_made_under_its_path(qtbot):
    rows = [_image("i2", "a dog", 50, 2), _i2v_video("v1", "styleA"), _image("i1", "a cat", 50, 1)]
    view = GalleryView(FakeDB(rows))
    qtbot.addWidget(view)
    view.refresh()
    workflow = _image_workflow(view._tree)
    ...
    assert view._title.display_text() == (
        f"{ORIENTATION_LABELS[LANDSCAPE]}  ›  All  ›  {workflow.text(0)}  ›  Latest")


def test_a_folders_shelf_names_itself_by_its_path_to_the_slideshow_button(qtbot):
    view = GalleryView(FakeDB([_image("i1", "a cat", 50, 1)]))
    qtbot.addWidget(view)
    view.refresh()
    workflow = _image_workflow(view._tree)
...
def test_recents_shelf_is_pinned_first_and_lists_recent_items(qtbot):
    ...
    assert list(_alls(view._tree))[0] == "Latest"
...
def test_double_clicking_a_recent_item_opens_it_selected_in_its_folder(qtbot):
    ...
    assert view.selected_generation() == "i2"
    assert view._browser.showing_recents() is False             # off the shelf...
    ...
    assert view._browser._thumb_widgets["i2"].is_selected()
...
def _right_click(view, prompt_id):
    ...
    view._browser._thumb_widgets[prompt_id].customContextMenuRequested.emit(QPoint(0, 0))
...
def test_media_filter_hides_the_unchecked_media_type_on_a_shelf(qtbot):
    rows = [_image("i1", "a cat", 50, 1), _i2v_video("v1", "styleA")]
    view = GalleryView(FakeDB(rows))
    qtbot.addWidget(view)
    view.refresh()
    ...
    view._video_cb.setChecked(False)          # hide videos
    assert view.visible_prompt_ids() == ["i1"]

    view._video_cb.setChecked(True)
    view._image_cb.setChecked(False)          # hide images instead
    assert view.visible_prompt_ids() == ["v1"]

    view._video_cb.setChecked(False)          # both off: nothing to show
    assert view.visible_prompt_ids() == []
...
def test_media_filter_prunes_the_folder_tree_as_well_as_the_pane(qtbot):
    ...
    qtbot.addWidget(view)
    view.refresh()
    assert len(_workflow_rows(view._tree)) == 2

    view._video_cb.setChecked(False)

    assert list(_workflow_rows(view._tree)) == ["SDXL Text-to-Image"]
...
def test_a_new_media_filter_reopens_recents_on_its_first_page(qtbot):
    ...
    view._video_cb.setChecked(False)  # images only — still all 120 of them

    assert len(view.visible_prompt_ids()) == 50
    assert view.visible_prompt_ids()[0] == "i119"


def test_media_filter_also_hides_inflight_cards_of_the_unchecked_type(qtbot):
    ...
    view.refresh()
    view._tree.setCurrentItem(_alls(view._tree)["Latest"])
    assert "rr1" in view._browser._inflight_cards               # shown by default

    view._video_cb.setChecked(False)           # hide videos
    assert "rr1" not in view._browser._inflight_cards
...
def test_recents_video_tiles_animate_while_images_stay_still(qtbot, tmp_path, monkeypatch):
    ...
    assert view._browser._thumb_widgets["v1"].findChildren(QMovie)          # the video loops
    assert view._browser._thumb_widgets["i1"].findChildren(QMovie) == []    # the image is a still
...
def test_settings_folder_video_tiles_animate(qtbot, tmp_path, monkeypatch):
    ...
    assert view._browser._thumb_widgets["v1"].findChildren(QMovie)  # its grid tile loops
...
def test_recents_tiles_are_badged_image_or_video(qtbot):
    ...
    def badge_type(pid):
        badges = view._browser._thumb_widgets[pid].findChildren(MediaBadge)
...
def test_new_generations_appear_without_manual_refresh(qtbot):
    ...
    qtbot.addWidget(view)
    view.refresh()
    assert set(_top_level(view._tree)) == {"All"}
    assert set(_workflow_rows(view._tree)) == {"SDXL Text-to-Image"}
...
def test_folders_start_collapsed(qtbot):
    ...
    workflow = _image_workflow(view._tree)
    assert workflow.isExpanded() is False
    assert _at(workflow, 0).isExpanded() is False
...
def test_a_folder_named_after_what_it_holds_offers_no_rename(qtbot):
    # A workflow, a model and a LoRA are named by the fact they stand for;
    # renaming one would only hide which one it is. Neither the tree row, the
    # header, nor the context menu opens an editor on them.
    db = FakeDB([_image("i1", "a cat", 50, 1)])
    view = GalleryView(db)
    qtbot.addWidget(view)
    view.refresh()
    ...
    model = _at(workflow, 0)
    lora = _at(model, 0)
    for item in (workflow, model, lora):
...
def _video_leaf(view):
    """Open the settings folder under the video workflow, its deepest branch's
    only leaf."""
    item = _video_workflow(view._tree)
...
def test_a_star_pressed_on_one_tile_bookmarks_it_at_once_and_leaves_the_rest(qtbot):
    ...
    qtbot.addWidget(view)
    view.refresh()
    view._tree.setCurrentItem(_alls(view._tree)["Latest"])  # both on screen
...
def test_the_favorites_count_follows_a_favorite_without_rebuilding_the_gallery(qtbot, monkeypatch):
    ...
    view.favorite_generation("i1")
    assert _shown(_alls(view._tree)["Favorites"]) == "(1) Favorites"

    view.set_items_favorite(["i1"], False)
    assert _shown(_alls(view._tree)["Favorites"]) == "Favorites"
    assert rebuilds == []
...
def test_a_tiles_trash_corner_deletes_that_tile(qtbot):
    ...
    view._browser._thumb_widgets["i2"]._controls.triggered.emit(corner_controls.TRASH)
...
def test_the_enhance_corner_offers_another_the_moment_a_setting_moves(qtbot, tmp_path):
    ...
    assert view._browser._thumb_widgets["g0"].enhance_state() == icons.ENHANCE_MORE
...
def test_back_returns_to_a_shelf_left_by_opening_a_folder(qtbot):
    ...
    view._tree.setCurrentItem(
        _at(_image_workflow(view._tree), 0, 0, 0)
    )
    assert view._browser.showing_recents() is False
...
def test_back_returns_to_the_favorite_shelf_after_drilling_into_a_folder(qtbot):
    ...
    qtbot.addWidget(view)
    view.refresh()
    dog_key = _key(_at(_image_workflow(view._tree), 0, 0, 1))
    view._toggle_favorite(dog_key)
...
def test_select_folder_restores_choice_in_a_fresh_view(qtbot):
    ...
    dog_leaf = _at(_image_workflow(saved._tree), 0, 0, 1)
...
def test_double_clicking_a_thumbnail_pins_the_tab_it_opened(qtbot, tmp_path):
    ...
    view._browser._thumbnail_double_clicked("p1")
...
def test_double_clicking_an_unregistered_thumbnail_still_pins_its_tab(qtbot, tmp_path):
    ...
    view._browser._thumbnail_double_clicked("unreg")
...
def _open_leaf(view):
    ...
    workflow = _image_workflow(view._tree)
    leaf = _at(workflow, 0, 0, 0)  # workflow -> model -> "(no add-on)" -> settings
    view._tree.setCurrentItem(leaf)
    return leaf
...
def test_ctrl_click_extends_selection_and_delete_takes_all_picked(qtbot):
    ...
    view._browser.apply_selection("i1", _NO_MOD)
    view._browser.apply_selection("i3", _CTRL)
...
def test_ctrl_click_again_deselects_a_tile(qtbot):
    ...
    view._browser.apply_selection("i1", _NO_MOD)
    view._browser.apply_selection("i2", _CTRL)
    view._browser.apply_selection("i2", _CTRL)  # toggle i2 back off
...
def test_shift_click_selects_the_contiguous_range(qtbot):
    ...
    view._browser.apply_selection(order[0], _NO_MOD)
    view._browser.apply_selection(order[2], _SHIFT)
...
def test_plain_click_replaces_the_whole_selection(qtbot):
    ...
    view._browser.apply_selection("i1", _NO_MOD)
    view._browser.apply_selection("i2", _CTRL)     # now both selected
    view._browser.apply_selection("i2", _NO_MOD)   # plain click collapses to one
    assert view.selected_prompt_ids() == ["i2"]
...
def test_changing_folders_clears_the_selection(qtbot):
    ...
    dog_leaf = _at(cat_leaf.parent(), 1)
...
def test_clicking_the_pane_background_leaves_the_tiles_on_screen(qtbot):
    ...
    assert set(view.visible_prompt_ids()) == {"i1", "i2"}
    assert not any(w.is_selected() for w in view._browser._thumb_widgets.values())
...
def test_clicking_a_thumbnail_keeps_its_press_to_itself(qtbot):
    ...
    qtbot.mouseClick(view._browser._thumb_widgets["i1"], Qt.MouseButton.LeftButton)
    assert view.selected_prompt_ids() == ["i1"]
...
def test_right_clicking_an_unpicked_thumbnail_selects_it(qtbot, monkeypatch):
    ...
    view._browser._thumbnail_context_menu("i2", QPoint(0, 0))
...
def test_right_click_delete_acts_on_the_whole_multi_selection(qtbot, monkeypatch):
    ...
    _open_leaf(view)
    view._browser.apply_selection("i1", _NO_MOD)
    view._browser.apply_selection("i3", _CTRL)  # i1 + i3 selected
    monkeypatch.setattr(
        "origenerator.gui.gallery_view.QMenu.exec", lambda self, *a: self.actions()[-1]
    )

    # Right-clicking a tile already in the selection keeps the whole set.
    view._browser._thumbnail_context_menu("i3", QPoint(0, 0))
...
def test_right_clicking_a_selected_tile_preserves_the_multi_selection(qtbot, monkeypatch):
    ...
    _open_leaf(view)
    view._browser.apply_selection("i1", _NO_MOD)
    view._browser.apply_selection("i3", _CTRL)  # i1 + i3
    monkeypatch.setattr("origenerator.gui.gallery_view.QMenu.exec", lambda self, *a: None)

    # A real right-press on a selected tile must not collapse the selection to one.
    qtbot.mouseClick(view._browser._thumb_widgets["i3"], Qt.MouseButton.RightButton)
...
def test_deleting_a_folder_lands_on_the_parent_not_the_top(qtbot, tmp_path):
    ...
    qtbot.addWidget(view)
    view.refresh()
    model = _at(_image_workflow(view._tree), 0)
    view._tree.setCurrentItem(_at(model, 0, 0))  # one of the two settings leaves
...
def test_deleting_a_folder_returns_to_the_most_recent_one_still_there(qtbot, tmp_path):
    ...
    qtbot.addWidget(view)
    view.refresh()
    lora = _at(_image_workflow(view._tree), 0, 0)  # the (no add-on) folder
    first_leaf, second_leaf = _at(lora, 0), _at(lora, 1)
...
def test_delete_folder_refuses_a_workflow_group(qtbot):
    actions = FakeActions()
    view = GalleryView(FakeDB([_image("i1", "a cat", 50, 1)]), actions=actions)
    qtbot.addWidget(view)
    view.refresh()
    ...
    view._delete_folder(_image_workflow(view._tree).data(0, _GROUP_ROLE))
...
def test_the_hover_rows_trash_icon_deletes_the_folder_it_names(qtbot, monkeypatch):
    ...
    view._tree.delete_clicked.emit(_key(_at(lora, 0)))
...
def test_a_lora_folder_is_deletable_and_takes_only_its_own_rows(qtbot):
    ...
    qtbot.addWidget(view)
    view.refresh()
    # WAN I2V -> model -> LoRA (styleA is first by appearance).
    lora = _at(_video_workflow(view._tree), 0, 0)
...
def test_the_new_folder_lists_its_images_in_the_order_the_old_one_reads(qtbot, tmp_path):
    ...
    view.refresh()
    key = _select_first_leaf(view)
    # _tree_item_for: a folder has a row per side now, and this key has no
    # side on it.
    was = [row["seed"] for row in view._tree_item_for(key).data(0, _GROUP_ROLE).rows]
...
def test_turning_auto_on_in_a_second_folder_ends_the_first_ones_loop(qtbot, tmp_path):
    ...
    qtbot.addWidget(view)
    view.refresh()
    settings = _at(_image_workflow(view._tree), 0, 0)
    first = _at(settings, 0).data(0, _GROUP_ROLE).key
    view._tree.setCurrentItem(_at(settings, 0))
    view._toggle_auto(True)

    view._tree.setCurrentItem(_at(settings, 1))
    view._toggle_auto(True)
...
def test_favorite_slideshow_plays_favorite_items_and_folders_once(qtbot, monkeypatch):
    ...
    view.refresh()
    lora = _at(_image_workflow(view._tree), 0, 0)  # "(no add-on)"
    view._toggle_favorite(_key(_at(lora, 0)))   # ...and a favorited folder (the cat one)
    view._tree.setCurrentItem(_shelf(view, FAVORITES_KEY))
...
def _tooltips(view, prompt_id):
    return [b.toolTip() for b in view._browser._thumb_widgets[prompt_id]._corner_buttons]
...
def test_pressing_delete_after_clicking_a_thumbnail_removes_it(qtbot, tmp_path):
    ...
    _open_leaf(view)
    thumb = view._browser._thumb_widgets["i1"]
...
def test_delete_works_with_gallery_as_the_central_widget(qtbot, tmp_path):
    ...
    view.refresh()
    view._tree.setCurrentItem(_at(_image_workflow(view._tree), 0, 0, 0))
    view._browser.apply_selection("i1", _NO_MOD)
...
def test_recents_shows_an_inflight_card_above_finished_items(qtbot):
    ...
    assert "gen1" in view._browser._inflight_cards
...
def test_recents_shows_a_live_reroll_as_an_inflight_card(qtbot):
    ...
    qtbot.addWidget(view)
    view.refresh()
    folder_key = _key(_at(_image_workflow(view._tree), 0, 0))
    ...
    assert not view._browser._inflight_cards["rr1"]._image.pixmap().isNull()  # the job's live frame
    # Clicking the re-roll card opens the folder it runs in (its tile shows there).
    view._browser._on_inflight_clicked("rr1")
...
def test_right_clicking_an_inflight_card_offers_to_cancel_the_run(
    ...
    _open_recents(view)
    assert job.prompt_id in view._browser._inflight_cards
...
def test_a_card_with_no_live_job_behind_it_raises_no_menu(qtbot, monkeypatch):
    ...
    view.refresh()
    _open_recents(view)
    assert "gen1" in view._browser._inflight_cards      # the card is there...
    labels = _menu_offering(monkeypatch, "browser_pane")
...
def test_inflight_card_frame_updates_in_place_without_a_rerender(qtbot):
    ...
    view._poll()
    assert view._browser._inflight_cards["rr1"] is card       # same card object — no re-render
...
def test_recents_shelf_appears_for_a_running_generation(qtbot):
    ...
    assert "Latest" in _alls(view._tree)
    _open_recents(view)
    assert "gen1" in view._browser._inflight_cards
...
def test_generate_inflight_card_persists_across_navigation_and_polls(qtbot, tmp_path):
    ...
    folder = _at(_image_workflow(gv._tree), 0, 0)
    gv._tree.setCurrentItem(folder)              # navigate away
    gv._tree.setCurrentItem(_shelf(gv, RECENTS_KEY))    # and back
    assert pid in gv._browser._inflight_cards, "card vanished after navigating away and back"

    gv._poll()
    assert pid in gv._browser._inflight_cards, "card vanished after a poll"
...
def test_reroll_inflight_card_persists_across_navigation_and_polls(qtbot, tmp_path):
    ...
    gv._tree.setCurrentItem(_shelf(gv, RECENTS_KEY))             # and back
    assert rr_pid in gv._browser._inflight_cards, "re-roll card vanished after navigation"

    gv._poll()
    assert rr_pid in gv._browser._inflight_cards, "re-roll card vanished after a poll"
...
def test_i2v_reroll_inflight_card_follows_the_image_to_video_handoff(qtbot, tmp_path):
    ...
    view._poll()
    assert vid_job.prompt_id in view._browser._inflight_cards, "video-stage card missing after handoff"

    view._tree.setCurrentItem(view._tree_item_for(key))   # away
    view._tree.setCurrentItem(_shelf(view, RECENTS_KEY))        # and back
    assert vid_job.prompt_id in view._browser._inflight_cards, "video-stage card missing after navigation"
...
def test_recents_shows_a_running_reroll_row_with_no_live_job(qtbot, tmp_path):
    ...
    assert "rr_running" in gv._browser._inflight_cards
...
def test_a_draggable_divider_parts_the_hud_from_the_enhance_settings(qtbot, tmp_path):
    ...
    assert divider.handleWidth() == view._arrangement.panes.handleWidth()
...
def test_a_running_enhance_gets_no_card_of_its_own_on_recents(qtbot, tmp_path):
    ...
    assert job.prompt_id not in view._browser._inflight_cards
    assert view._browser._thumb_widgets["g0"]._enhancing is not None
...
def test_combine_new_folder_lands_on_recents_then_reveals_on_finish(qtbot, tmp_path):
    ...
    assert not view._browser.showing_recents()  # the finished row gave its folder a node
...
def test_the_consoles_control_group_is_the_apps_one_osr2_switch(qtbot):
    ...
    assert view.osr2_control.isChecked() is False
...
def test_a_hold_from_the_console_stills_the_motion_and_keeps_control(qtbot):
    ...
    assert view.osr2_control.isChecked() is True
...
def test_global_toggle_drives_the_front_video_and_untoggling_stops(qtbot):
    ...
    view.osr2_control.setChecked(True)  # the one global switch, on
    assert driver.started == [("player-A", "actions-A")]

    view.osr2_control.setChecked(False)  # off
    assert driver.stopped == 1
...
def test_toggle_on_with_no_video_shown_moves_instead(qtbot):
    ...
    view.osr2_control.setChecked(False)
...
def test_space_flips_the_one_switch_rather_than_the_motion_alone(qtbot, monkeypatch):
    ...
    assert not view.osr2_control.isChecked()      # the switch went off, not the motion
...
def test_the_control_state_round_trips_for_persistence(qtbot):
    view, _driver, _panel = _osr2_view(qtbot)
    assert view.osr2_state() == OSR2_CONTROL_OFF

    for state in (OSR2_PARKED, OSR2_RETRACTED, OSR2_DRIVING, OSR2_CONTROL_OFF):
        view.restore_osr2_state(state)
        assert view.osr2_state() == state, state
...
def test_esc_stops_osr2_driving(qtbot):
    ...
    panel.osr2_drive_target = lambda: ("A.mp4", "pA", "aA")
    view.osr2_control.setChecked(True)  # driving the device
    assert driver.started

    handled = _press_escape(view)

    assert handled is True
    assert view.osr2_control.isChecked() is False and not view.osr2_control.isChecked()
    assert driver.stopped >= 1
...
def test_esc_puts_back_a_motion_that_was_running_without_the_switch(qtbot,
    ...
    _press_escape(view)
    assert view._osr2_motion.active and not view.osr2_control.isChecked()
...
def _two_leaf_view(qtbot, extra=(), actions=None):
    ...
    view.refresh()
    lora = _at(_image_workflow(view._tree), 0, 0)
    return view, _key(_at(lora, 0)), _key(_at(lora, 1))
...
def test_a_custom_folder_leads_with_how_many_items_it_gathers_counting_each_once(qtbot):
    ...
    lora = _key(_at(_image_workflow(view._tree), 0, 0))
...
def test_dropping_a_folder_onto_favorite_favorites_it(qtbot):
    ...
    view._on_folders_dropped(FAVORITES_KEY, [cat])
...
def test_right_clicking_outside_a_selection_is_about_the_row_under_the_cursor(
    ...
    fish = _key(_at(_image_workflow(view._tree), 0, 0, 2))
    _pick(view, cat, dog)
    labels = []
    _menu_labels(monkeypatch, labels)
...
def test_the_tree_carries_a_requests_shelf(qtbot):
    ...
    assert "Requests" in _alls(view._tree)
...
def test_the_shelf_row_counts_what_is_waiting(qtbot, tmp_path, monkeypatch):
    ...
    assert _shown(_alls(view._tree)["Requests"]) == "(1) Requests"
...
@pytest.mark.parametrize("said, key", [
    ("experiments", EXPERIMENTS_KEY),
    ("trash", TRASH_KEY),
    ("recents", RECENTS_KEY),
    ("go to favorites", FAVORITES_KEY),
    ("go to experiments", EXPERIMENTS_KEY),
])
...
def test_the_requests_shelf_answers_the_plural_and_the_singular_still_dictates(
    ...
    assert _selected_folder(view) == REQUESTS_KEY
...
def test_right_clicking_a_folder_tile_raises_the_folder_menu(qtbot, monkeypatch):
    ...
    labels = []
    _menu_labels(monkeypatch, labels)
    model = _key(_at(_image_workflow(view._tree), 0))
    (tile,) = [tile for tile in view._scroll.widget().findChildren(FolderTile)
               if tile.key == model]

    tile.context_requested.emit(model, QPoint(1, 1))

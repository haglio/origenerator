...
# --- the permanent Inspect tab + config-tab basics -------------------------

def test_starts_with_only_the_inspect_tab(tabs):
    assert tabs.count() == 1
    assert tabs.widget(0) is tabs._inspect_page
...
def test_config_tabs_are_closable_the_inspect_tab_is_not(tabs):
    # Installing a custom bar must precede setTabsClosable, or the per-tab close
    # button silently vanishes — this guards that ordering — while the always-
    # present Inspect tab (index 0) has its close button stripped.
    from PyQt6.QtWidgets import QTabBar
    tabs._add_subtab()  # a config tab at index 1
    bar = tabs.tabBar()
    ...
    assert close_btn(1) is not None  # a config tab is closable
    assert close_btn(0) is None      # the Inspect tab is not


def test_thumbnail_strip_sits_within_the_tabbed_content(tabs):
    # A config tab's strip lives inside the tab's content, so the tab row spans it
    # just like the preview and settings — it isn't a sibling of the tab bar.
...
def test_closing_the_last_config_tab_leaves_the_inspect_tab(tabs):
    panel = tabs._add_subtab()
...
def test_strip_click_opens_new_tab_when_settings_differ(tabs):
    ...
    tabs._add_subtab()  # a blank config tab — a different settings folder than g1
    tabs._on_strip_activated("g1")
    assert tabs.count() == 3  # Inspect + the blank tab + a new one for g1
    panel = tabs.currentWidget()
...
def test_strip_click_does_nothing_when_settings_match(tabs):
    ...
    tabs.open_config("sdxl_t2i", params)  # active tab now has g1's settings
    count = tabs.count()
    tabs._on_strip_activated("g1")
...
def test_opening_tab_from_a_thumbnail_populates_strip_with_its_folder(tabs):
    ...
    tabs._add_subtab()  # a blank tab (different folder) so the click opens a new one
...
def test_inspect_tab_is_not_renamable(tabs, monkeypatch):
    from PyQt6.QtWidgets import QInputDialog
    opened = []
    monkeypatch.setattr(QInputDialog, "getText",
                        lambda *a, **k: opened.append(True) or ("X", True))
    tabs._rename_subtab(0)  # double-clicking the Inspect tab
    assert opened == []
    assert tabs.tabText(0) == "Inspect"
...
def test_double_clicking_close_does_not_open_rename(tabs, monkeypatch):
    ...
    from PyQt6.QtWidgets import QInputDialog
    tabs._add_subtab()
    tabs._add_subtab()  # two config tabs, so a neighbor slides under the cursor
    opened = []
    monkeypatch.setattr(QInputDialog, "getText",
                        lambda *a, **k: opened.append(True) or ("X", True))
...
def test_capture_state_lists_config_tabs_and_current(tabs):
    tabs._add_subtab()                                            # sdxl at index 1
    tabs.open_config("wan22_i2v", {"positive_prompt": "a fox"})   # index 2, current
    state = tabs.capture_state()
    workflows = [tab["config"]["workflow_name"] for tab in state["tabs"]]
    assert workflows == ["sdxl_t2i", "wan22_i2v"]
    assert state["current"] == 2  # overall index (0 is Inspect)
...
def test_restore_state_rebuilds_config_tabs(tabs):
    ...
    assert len(panels) == 2  # the pre-existing config tab was replaced
...
def test_restore_state_leaves_only_inspect_when_nothing_restorable(tabs):
    tabs.restore_state({})
...
def test_capture_restore_round_trips_config_and_custom_title(tabs, qtbot):
    tabs._add_subtab()  # a plain sdxl tab

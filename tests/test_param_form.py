...
def _stub_show_in_explorer(monkeypatch):
    """Replace the OS reveal so tests never spawn Explorer; returns the list of
    paths it was asked to reveal."""
    import origenerator.gui.param_form as pf

    revealed = []
    monkeypatch.setattr(pf, "show_in_explorer", lambda path: revealed.append(path))
    return revealed


def test_image_field_gets_a_show_in_explorer_button(qtbot):
    form = ParamForm([ParamDef("input_image", "Input Image", "image", "")])
    qtbot.addWidget(form)
    assert isinstance(form._reveal_buttons["input_image"], QPushButton)


def test_non_image_fields_get_no_show_in_explorer_button(qtbot):
    form = ParamForm([
        ParamDef("seed", "Seed", "seed", 0),
        ParamDef("steps", "Steps", "int", 20),
        ParamDef("name", "Name", "str", ""),
    ])
    qtbot.addWidget(form)
    assert form._reveal_buttons == {}


def test_show_in_explorer_button_sits_after_browse(qtbot):
    # The row reads [field] [Browse...] [Show in Explorer]: browse changes the
    # value, reveal acts on it, so reveal trails.
    form = ParamForm([ParamDef("input_image", "Input Image", "image", "")])
    qtbot.addWidget(form)
    cell = _field_cell_of(form, "input_image")
    browse_i = cell.indexOf(form._browse_buttons["input_image"])
    reveal_i = cell.indexOf(form._reveal_buttons["input_image"])
    assert 0 <= browse_i < reveal_i


def test_show_in_explorer_reveals_the_current_file(qtbot, monkeypatch, tmp_path):
    revealed = _stub_show_in_explorer(monkeypatch)
    img = tmp_path / "cat.png"
    img.write_bytes(b"\x89PNG")

    form = ParamForm([ParamDef("input_image", "Input Image", "image", str(img))])
    qtbot.addWidget(form)
    form._reveal_buttons["input_image"].click()

    assert revealed == [img]


def test_show_in_explorer_resolves_a_bare_name_against_the_input_dir(
    qtbot, monkeypatch, tmp_path
):
    import origenerator.gui.param_form as pf

    monkeypatch.setattr(pf, "COMFYUI_INPUT_DIR", tmp_path)
    (tmp_path / "preset.png").write_bytes(b"\x89PNG")
    revealed = _stub_show_in_explorer(monkeypatch)
    ...
    # A bare name resolves against ComfyUI's input folder, as LoadImage does.
    assert revealed == [tmp_path / "preset.png"]


def test_show_in_explorer_is_disabled_until_the_field_names_a_real_file(
    qtbot, monkeypatch, tmp_path
):
    import origenerator.gui.param_form as pf

    monkeypatch.setattr(pf, "COMFYUI_INPUT_DIR", tmp_path)  # keep bare names from matching
    img = tmp_path / "cat.png"
    img.write_bytes(b"\x89PNG")

    form = ParamForm([ParamDef("input_image", "Input Image", "image", "")])
    qtbot.addWidget(form)
    btn = form._reveal_buttons["input_image"]
    # Empty field: nothing to reveal, so the button is greyed out.
    assert btn.isEnabled() is False

    form._widgets["input_image"].setText(str(img))
    assert btn.isEnabled() is True   # a real file enables it live

    form._widgets["input_image"].setText("does-not-exist.png")
    assert btn.isEnabled() is False  # and a bad path disables it again


def test_show_in_explorer_does_nothing_when_the_field_is_empty(qtbot, monkeypatch):
    revealed = _stub_show_in_explorer(monkeypatch)

    form = ParamForm([ParamDef("input_image", "Input Image", "image", "")])
    qtbot.addWidget(form)
    form._reveal_image("input_image")  # the button is disabled, but guard anyway

...
class GenerateButton(QPushButton):
    ...
    ``start`` puts it in progress mode (still pressable, still reading "Generate",
    filling from 0); ``set_progress`` advances the fill; ``finish`` restores the
    idle button. ``flash_guard`` briefly shows a guard message while idle.
    """

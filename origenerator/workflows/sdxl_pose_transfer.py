    ...
    def param_definitions(self) -> list[ParamDef]:
        ...
        return [
            ...
            ParamDef("checkpoint", "Model", ParamType.COMBO, defaults["checkpoint"],
                     options=checkpoints),
            ParamDef("control_mode", "Structure From", ParamType.COMBO, defaults["control_mode"],
                     options=["depth", "pose"]),
            ParamDef("controlnet", "Structure Model", ParamType.COMBO, defaults["controlnet"],
                     options=controlnets),
            ParamDef("controlnet_strength", "Structure Strength", ParamType.FLOAT, defaults["controlnet_strength"],
                     min_val=0.0, max_val=2.0, step=0.05),
            ParamDef("controlnet_end", "Structure Hold", ParamType.FLOAT, defaults["controlnet_end"],
                     min_val=0.0, max_val=1.0, step=0.05),
            ParamDef("seed", "Seed", ParamType.SEED, defaults["seed"]),
            ParamDef("steps", "Steps", ParamType.INT, defaults["steps"], min_val=1, max_val=200),
            ParamDef("cfg", "Prompt Strength", ParamType.FLOAT, defaults["cfg"], min_val=0.0, max_val=30.0, step=0.5),
            ParamDef("sampler_name", "Drawing Method", ParamType.COMBO, defaults["sampler_name"],
                     options=SAMPLER_OPTIONS),
            ParamDef("scheduler", "Step Spacing", ParamType.COMBO, defaults["scheduler"],
                     options=SCHEDULER_OPTIONS),
            ParamDef("denoise", "Redraw Amount", ParamType.FLOAT, defaults["denoise"], min_val=0.0, max_val=1.0, step=0.01),

...
class ShowSet:
    ...
    def reorder(self, items, *, latest: bool, enhanced_ids=()) -> None:
        ...
        self.wear(HudFacts(order_label=LATEST_LABEL if latest else SHUFFLE_LABEL,
                           starred_ids=self.starred_ids, enhanced_ids=enhanced_ids))
    ...
    def filter_to(self, query: str) -> bool:
        """Narrow to one configuration, the way a satellite narrows to one act
        — the button at the head of a map row: the cell that row belongs to is
        jumped to and its seed row looped.  ``False`` when no row is labeled
        *query*, or that row holds only the one slide."""
        shown = self.map()
        if shown is None:
            return False
        rows = ((shown.corner, shown.label), *zip(shown.configs, shown.config_labels))
        for slide, label in rows:
            if label and label_query(label) == query:
                self.jump_to(slide)
                return self.start_loop(SEED_AXIS)
        return False
    ...
    def unstar_current(self, unstar) -> bool:
        """Take the star off the item on screen, through *unstar* — the
        ...
        if unstar is None or prompt_id is None or prompt_id not in self.starred_ids:
            return False
        unstar(prompt_id)
        self.starred_ids.discard(prompt_id)  # the star readout and F-mode follow it
        return True

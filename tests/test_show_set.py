...
def test_a_switch_ends_a_loop_and_deals_the_browse():
    show_set, _dealt = _mapped(hud=HudFacts(starred_ids={"id-2"}))
    show_set.start_loop("seed")
...
def test_a_rows_filter_button_narrows_to_that_configuration():
    """The button at the head of a map row — a satellite's act filter — is
    the configuration filter here: the row's cell is jumped to and its seed
    row looped, spelled the way the panel posts it."""
    show_set, _dealt = _mapped()

    assert show_set.filter_to("dawn") is False       # id-1x has no seed row of its own
    assert show_set.current_prompt_id() == "id-1x"    # but it was jumped to
    show_set.jump_to(show_set.slide_for_path("one.png"))
    assert show_set.filter_to("fox") is True

    assert show_set.map().loop == "seed"
    assert show_set.filter_to("nobody") is False

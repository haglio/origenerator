"""Deriving an i2v workflow's output size from its input image.

Every image-to-video workflow keeps the input image's aspect ratio at a fixed
pixel budget rather than a hardcoded resolution. The WAN 2.2 pair does this
in-graph (``ImageScaleToTotalPixels`` at 0.4 MP on a /16 stride, then
``GetImageSize``); ATI can't (its ``WanTrackToVideo`` needs the integer size
*and* a track whose coordinates share that space, both built app-side). This
module is the app-side twin of that in-graph scaling: it replicates
``ImageScaleToTotalPixels`` exactly, so the size it computes for an image equals
what the in-graph path produces for the same image.

It's used two ways: ATI builds its payload size from it, and the Generate form
shows every i2v workflow's derived size in a locked Dimensions field (measuring
...
# The pixel budget and stride the WAN 2.2 workflows scale to in-graph, matched
# here so an image yields the same proportions across every i2v workflow.
...
def scale_to_total_pixels(src_width: int, src_height: int) -> tuple[int, int]:
...
def measure_derived_size(input_image: str | None) -> tuple[int, int] | None:
    ...
    image is missing or unreadable — so a caller can fall back (ATI to its
    reference frame, the form to showing no size) rather than crash.
    """
    ...
    try:
        with Image.open(path) as img:
            return scale_to_total_pixels(*img.size)

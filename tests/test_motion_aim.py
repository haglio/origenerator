...
def test_detect_grip_aim_picks_the_most_confident_anchor(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(motion_aim, "_detect", lambda path: [
        {"class": "LABEL_OTHER", "score": 0.9, "rect": (0, 0, 10, 10)},
        {"class": "ANCHOR_EXPOSED", "score": 0.4, "rect": (10, 20, 30, 40)},
        {"class": "ANCHOR_EXPOSED", "score": 0.8, "rect": (50, 100, 60, 200)},
    ])
    aim = motion_aim.detect_grip_aim(frame)
...
def test_detect_grip_aim_returns_none_when_nothing_usable(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(motion_aim, "_detect", lambda path: [
        {"class": "ANCHOR_EXPOSED", "score": 0.2, "rect": (1, 1, 2, 2)},  # sub-threshold
    ])
    assert motion_aim.detect_grip_aim(frame) is None
    assert motion_aim.detect_grip_aim(None) is None

    def boom(path):
        raise RuntimeError("model missing")

    monkeypatch.setattr(motion_aim, "_detect", boom)
    assert motion_aim.detect_grip_aim(frame) is None  # best-effort, never raises

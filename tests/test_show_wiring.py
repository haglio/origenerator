...
def test_the_hud_facts_travel_as_one_record(qtbot):
    view = SlideshowView(_ITEMS, player=MagicMock(), shuffle=lambda order: None,
                         hud=HudFacts(order_label="Latest", starred_ids={"id-b"}))
    qtbot.addWidget(view)
...
def test_the_hud_facts_default_to_a_shuffled_set_of_nothing_starred(qtbot):
...
def test_retuning_a_show_dresses_it_as_a_base_state(qtbot):
    ...
    # that out and could have disagreed; there is one answer now, and the stars
    # it already had are not part of it.
    view = SlideshowView(_ITEMS, player=MagicMock(), shuffle=lambda order: None,
                         hud=HudFacts(order_label="Latest", starred_ids={"id-c"}))
    qtbot.addWidget(view)
